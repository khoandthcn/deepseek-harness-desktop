...
//
// The preset is the shipped `standard` composition with four rows appended:
...
export function writeSocPreset(upstream, output, repoRoot = resolve(import.meta.dirname, '..')) {
  ...
  text = `# Generated by deepseek-harness-desktop/scripts/make-soc-preset.mjs from the\n# upstream \`standard\` preset: WSO2 login service, read-only SOAR and EDR tools,\n# and a best-effort SIEM access probe.\n\n${text}`

  mkdirSync(output, { recursive: true })
  writeFileSync(join(output, 'agent.cordis.yml'), text)
  writeFileSync(join(output, 'preset.yml'), [
    'name: SOC Cloud (the platform)',
    'description: Standard coding agent plus read-only the platform SOAR and EDR tools, a best-effort SIEM access probe, and soc_login; the OTP is asked for at run time.',
    'order: 3',
    '',
  ].join('\n'))
}
