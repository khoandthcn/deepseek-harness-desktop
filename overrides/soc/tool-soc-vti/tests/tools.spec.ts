...
function defs(configured: boolean, responses: Record<string, unknown> = {}) {
  ...
  const list = createVtiToolDefs({ http: http as unknown as VtiHttpLike, auth, now: () => NOW })
...
describe('createVtiToolDefs', () => {
  it('defines the read-only Threat Intelligence suite', () => {
    ...
    expect(list.map(d => d.name)).toEqual([
      ...
      'vti_search_cves',
      'vti_search_threat_reports',
      'vti_get_threat_report',
      'vti_search_document_breaches',
      'vti_get_document_breach',
      'vti_get_credit_card_leak',
      'vti_search_easm_assets',
      'vti_search_easm_issues',
      'vti_lookup_indicator',
    ])
  })
  ...
  it('defines no tool that writes, marks or exports', () => {
    ...
    const verbs = list.map(d => d.name.replace(/^vti_/, '').split('_')[0])
...
describe('the shared alert search', () => {
  it('sends the documented body and reports the platform total', async () => {
    ...
    const { http, byName } = defs(true, { [VTI_PATHS.compromisedSystem]: envelope([row], 50) })
    const out = await byName('vti_search_compromised_systems').execute({ keyword: '10.20.30.40', severity: [3, 4] })
  ...
  it('omits an empty keyword and an out-of-range severity rather than sending them', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.phishing]: envelope([]) })
    await byName('vti_search_phishing').execute({ keyword: '', severity: [9, 'x'] })
  ...
  it('takes explicit epoch bounds over last_seconds and clamps the page size', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.dataLeak]: envelope([]) })
    await byName('vti_search_data_leaks').execute({ time_from: 1000, time_to: 2000, last_seconds: 99, size: 5000, from: 40 })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toMatchObject({ time_from: 1000, time_to: 2000, size: VTI_MAX_SIZE, from: 40 })
  })

  it('reports a payload that carries no rows, naming the keys it got', async () => {
    const { byName } = defs(true, { [VTI_PATHS.impersonate]: { message: 'quota exceeded' } })
    await expect(byName('vti_search_impersonations').execute({})).rejects.toThrow(/no `data` array.*message/)
  })
})

describe('the endpoints that spell their fields differently', () => {
  it('sends cvss_level with a CVE search', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.cve]: envelope([{ name: 'CVE-2025-49619' }]) })
    await byName('vti_search_cves').execute({ keyword: 'redhat', cvss_level: ['high', 'critical'] })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toMatchObject({ keyword: 'redhat', cvss_level: ['high', 'critical'] })
  })

  it('sends tlp with a threat-report search', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.threatReport]: envelope([]) })
    await byName('vti_search_threat_reports').execute({ tlp: [2, 3] })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toMatchObject({ tlp: [2, 3] })
  })

  it('names the document-breach window and paging as that endpoint does', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.dataBreach]: { alerts: [{ id: 'b1' }], total: 3 } })
    const out = await byName('vti_search_document_breaches').execute({ from: 20, size: 50, status: [0, 1] })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toEqual({
      offset: 20,
      limit: 50,
  ...
  it('names the attack-surface window from_time/to_time', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.easmAsset]: envelope([]) })
    await byName('vti_search_easm_assets').execute({ type: 'domain', status_code: [403] })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toEqual({
      from: 0,
      size: VTI_DEFAULT_SIZE,
  ...
  it('passes the issue handling states and the timestamp the window applies to', async () => {
    const { http, byName } = defs(true, { [VTI_PATHS.easmIssue]: envelope([]) })
    await byName('vti_search_easm_issues').execute({ customer_status: [0, 3], data_type: 'alert_time' })
    const [, body] = callsOf(http.postJson)[0] as [string, Record<string, unknown>]
    expect(body).toMatchObject({ customer_status: [0, 3], data_type: 'alert_time' })
  })
})

describe('vti_lookup_indicator', () => {
  it('asks for enrichment by default and returns the detail', async () => {
    ...
    const { http, byName } = defs(true, { [VTI_PATHS.threatLookup]: { success: true, message: 'OK', detail } })
    const out = await byName('vti_lookup_indicator').execute({ value: 'example.com', entity_type: 'domain' })
  ...
  it('reports a lookup that carried no detail', async () => {
    const { byName } = defs(true, { [VTI_PATHS.threatLookup]: { success: false, message: 'not found' } })
    await expect(byName('vti_lookup_indicator').execute({ value: 'x', entity_type: 'ip' }))
...
describe('the detail reads', () => {
  it('reads one threat report by its code, escaped into the path', async () => {
    const report = { code_report: 'VTI_2025_0123', title: 'x' }
    const { byName } = defs(true, { '/discovery-service/api/v1/threat_report/VTI_2025_0123': report })
    expect(await byName('vti_get_threat_report').execute({ code_report: 'VTI_2025_0123' })).toEqual(report)
  })

  it('reads one card-leak alert by its id', async () => {
    const { byName } = defs(true, { '/discovery-service/api/v1/ccleak/69e0c73e': { id: '69e0c73e' } })
    expect(await byName('vti_get_credit_card_leak').execute({ ccleak_id: '69e0c73e' })).toEqual({ id: '69e0c73e' })
  })
})
